...
class OutputParsers:
    ...
    def parse_route(output: str):
        ...
        for line in output.splitlines():
            ...
            if not s:
                continue
            if re.match(r'^(Static|O_|O IA|S |D |B |ISIS|直连)', s):
                parts = s.split()
                if len(parts) >= 3:
...
class PreconditionRegistry:
    ...
    def verify(self, experiment: ExperimentState, required_ids: List[str], context: Optional[Dict[str, Any]] = None):
        ...
        for cid in required_ids:
            ...
            if not pre:
                reasons.append(f'缺失前置条件检查器: {cid}')
                continue
...
class RecoveryRegistry:
    ...
    def run(self, engine: 'ExperimentEngine', action_id: str, context: Dict[str, Any]):
        ...
        if not action:
            return {'success': False, 'error': f'未注册的恢复策略: {action_id}'}
...
class VerificationEngine:
    ...
    def run_checks(self, experiment: ExperimentState, path: str, checks: List[str], target_ip: Optional[str] = None):
        ...
        if not device:
            return [CheckResult(check_name='device_lookup', passed=False, detail='目标设备不存在于实验状态中')]
        if 'interface' in checks:
            ...
            results.append(CheckResult(check_name='interface', passed=any(i.is_up() for i in device.interfaces.values()), detail=f'已解析 {len(mp)} 个接口', evidence={'interfaces': mp}))
        if 'ospf' in checks:
            ...
            results.append(CheckResult(check_name='ospf', passed=bool(peers) and all(p.get('state','').lower()=='full' for p in peers), detail=f'邻居数量 {len(peers)}', evidence={'peers': peers}))
        if 'route' in checks:
            ok, out = self._exec(path, 'display ip routing-table')
            routes = OutputParsers.parse_route(out)
            device.routes = routes
            results.append(CheckResult(check_name='route', passed=bool(routes), detail=f'路由数量 {len(routes)}', evidence={'routes_sample': routes[:20]}))
        if 'reachability' in checks and target_ip:
            ...
            results.append(CheckResult(check_name='reachability', passed=passed, detail=f'目标地址={target_ip}', evidence={'output': out[:800]}))
...
class ExperimentEngine:
    ...
    def _register_defaults(self):
        ...
        reg.register('connected', '设备必须处于已连接状态', lambda e, c: (True, '') if (e.devices.get(c.get('path','')) or DeviceState(path='', name='')).connected else (False, '设备未连接'))
        reg.register('interface_ip_ready', '至少一个接口已配置 IP', lambda e, c: (True, '') if any(i.has_ip() for i in (e.devices.get(c.get('path','')) or DeviceState(path='', name='')).interfaces.values()) else (False, '缺少接口 IP 配置'))
        reg.register('vlan_exists', '目标 VLAN 已存在', lambda e, c: (True, '') if any(v.get('vlan_id')==c.get('vlan_id') for v in (e.devices.get(c.get('path','')) or DeviceState(path='', name='')).protocols.get('vlans', [])) else (False, '目标 VLAN 未创建'))
        reg.register('acl_direction_ready', 'ACL 需明确接口与方向', lambda e, c: (True, '') if c.get('interface') and c.get('direction') else (False, '缺少接口或方向'))

        planner = self.planner
        planner.register(ExperimentPhase('base', '基础环境准备', '进入系统视图并关闭干扰输出', lambda e, c: ['system-view', 'undo terminal monitor'], lambda e, c: [c['path']], ['connected'], []))
        planner.register(ExperimentPhase('interface', '接口与 IP 配置', '配置接口 IP 并确保接口在线', lambda e, c: [f"interface {c['interface']}", f"ip address {c['ip']} {c['mask']}", 'undo shutdown', 'quit'] if c.get('interface') and c.get('ip') and c.get('mask') else [], lambda e, c: [c['path']], ['connected'], ['interface']))
        planner.register(ExperimentPhase('ospf', 'OSPF 协议配置', '配置 OSPF 并校验邻居收敛', lambda e, c: [f"ospf {c.get('process_id', 1)}", f"area {c.get('area', 0)}"] + [f"network {n}" for n in c.get('networks', [])] + ['quit'], lambda e, c: c.get('paths', [c.get('path', '')]), ['connected', 'interface_ip_ready'], ['ospf']))
        planner.register(ExperimentPhase('acl', 'ACL 策略配置', '按接口方向下发 ACL 并验证命中', lambda e, c: ([f"acl number {c['acl_number']}"] + list(c.get('rules', [])) + ['quit'] if c.get('acl_number') and c.get('rules') else []) + ([f"interface {c['interface']}", f"traffic-filter {c['direction']} acl {c['acl_number']}", 'quit'] if c.get('interface') and c.get('direction') and c.get('acl_number') else []), lambda e, c: [c['path']], ['connected', 'acl_direction_ready'], ['interface']))
        planner.register(ExperimentPhase('verify', '综合验证', '执行通用检查与连通性测试', lambda e, c: [], lambda e, c: [c.get('path', '')], [], ['interface', 'route', 'reachability']))

        rec = self.recoveries
        rec.register('rollback_last', '回滚到上一阶段前置快照', lambda engine, ctx: engine.rollback((ctx.get('paths') or [ctx.get('path', '')])[0], 'rollback'))
        rec.register('refresh_interface_state', '刷新接口状态', lambda engine, ctx: {'checks': [asdict(c) for c in engine.verifier.run_checks(engine.get_experiment(ctx['experiment_id']), ctx.get('path', ''), ['interface'])]})
        rec.register('check_ospf_consistency', '校验 OSPF 一致性', lambda engine, ctx: {'checks': [asdict(c) for p in ctx.get('paths', []) for c in engine.verifier.run_checks(engine.get_experiment(ctx['experiment_id']), p, ['ospf'])]})
        rec.register('collect_diagnostics', '采集诊断信息', lambda engine, ctx: {'diagnostics': {cmd: engine._command(ctx.get('path', ''), cmd)[1][:1500] for cmd in ['display current-configuration', 'display ip interface brief', 'display logbuffer']}})
    ...
    def execute_phase(self, experiment_id: str, phase_id: str, context: Dict[str, Any], auto_verify: bool = True, dry_run: bool = False):
        ...
        if not ok:
            return {'success': False, 'error': '前置条件不满足', 'reasons': reasons}
    ...
    def describe_phase(self, phase_id: str):
        ...
        return next((p for p in phases if p['phase_id'] == phase_id), {'phase_id': phase_id, 'error': '未找到阶段'})
