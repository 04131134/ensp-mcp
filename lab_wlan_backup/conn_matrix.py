...
TARGETS = {
    'AC1(V10)': '10.1.1.1', 'AC2(V10)': '10.1.1.2', 'AR1(V10)': '10.1.1.11',
    'AR2(V10)': '10.1.1.12', 'AR3(V10)': '10.1.1.3',
...
def ping(src_port, tip):
    c = TelnetConnection('127.0.0.1', src_port); c.connect()
    c.send_cmd('screen-length 0 temporary'); time.sleep(0.2)
...
print('CONNECTIVITY MATRIX  (Reply-from /3)\n')
...
for sname, sport in SOURCES:
    ...
    for tname, tip in TARGETS.items():
        try:
            rcv, rl = ping(sport, tip)
        except Exception as e:
            rcv, rl = -1, str(e)[:30]
        results[sname][tname] = (rcv, rl)
        row += f'{rcv}'.rjust(8)
