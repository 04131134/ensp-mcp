# -*- coding: utf-8 -*-
"""Comprehensive AP1/AP2 console reboot: sync with \\r\\n, send reboot, answer YES
on 'Continue', with fallback re-send. Then wait + verify all 3 APs normal."""
...
def reboot_console(name):
    ...
    for _ in range(15):
        try:
            s.settimeout(1.0); d = s.recv(65536)
        except socket.timeout:
            d = b''
        if d: buf += d
        if 'Continue' in buf.decode('gbk', errors='ignore') and not sent:
            ...
            print(f"[{name}] YES sent on Continue", flush=True)
            break
    if not sent:
        print(f"[{name}] no Continue; fallback re-send", flush=True)
        s.settimeout(10); s.send(b'YES\r\n'); time.sleep(1.0); drain(s, 1.0)
        s.settimeout(10); s.send(b'reboot\r\n'); time.sleep(2.0)
        s.settimeout(10); s.send(b'YES\r\n'); time.sleep(2.0); drain(s, 2.0)
    s.close()
...
print("Waiting 90s for AP1/AP2 reboot + re-register ...", flush=True)
time.sleep(90)
