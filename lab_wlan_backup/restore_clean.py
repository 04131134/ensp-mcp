...
def send_block(port, cmds, label):
    ...
    for cmd in cmds:
        ...
        if '[Y/N]' in txt or '(Y/N)' in txt:
            c.sock.send(b'YES\r\n'); time.sleep(0.8)
            try: txt+=c.sock.recv(8192).decode(errors='replace')
            except: pass
        print(f'[{label}] {cmd} -> {txt[-80:].strip()}')
    c.close()

print('===== undo HSB on both ACs =====')
for name,port in [('AC1',2004),('AC2',2005)]:
    send_block(port, ['system-view','wlan','undo ac protect enable','return'], name)
