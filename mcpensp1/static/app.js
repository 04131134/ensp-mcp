...
+'<div class="dm"><span class="dp">端口 '+d.port+'</span>'+(dt!=='unknown'?'<span class="dt">'+dt+'</span>':'')+'</div></div></div>'
+'<div class="dar">'+(c?'<span class="cbg" onclick="swTerm(\''+esc(d.path)+'\')">已连接</span><button class="rnb" onclick="openRM(\''+esc(d.path)+'\',\''+esc(n)+'\')">重命名</button><button class="fnb" onclick="fetchDN(\''+esc(d.path)+'\')" title="获取真实名">&#8635;</button>'
:'<button class="cb" onclick="connDev('+d.port+',\''+esc(d.path)+'\')">连接</button>')+'</div>'
+(c?'<button class="db" onclick="discDev(\''+esc(d.path)+'\')">断开连接</button>':'')+'</div>'}).join('')}
function scanDevices(){var s=parseInt(document.getElementById('sP').value)||2000,e=parseInt(document.getElementById('eP').value)||2050;if(s<1||e>65535||s>e){log('端口范围无效','error');return}log('扫描 '+s+'-'+e+'...');socket.emit('scan',{start:s,end:e})}
function connDev(p,path){if(dCon[path])return;log('连接 '+(dNames[path]||path)+'...');socket.emit('connect_device',{port:p})}
function swTerm(path){if(dCon[path])swTerm2(path)}
function discDev(path){log('断开 '+(dNames[path]||path)+'...');socket.emit('disconnect_device',{path:path})}
function fetchDN(path){log('获取 '+path+' 真实名...');socket.emit('fetch_device_name',{path:path})}function addTerm(path,name){var tabs=document.getElementById('tabs');var tp=document.getElementById('tp-'+eId(path));if(tp)return;
var tab=document.createElement('div');tab.className='tab';tab.id='t-'+eId(path);
tab.innerHTML='<span>'+esc(name)+'</span><button class="cls" onclick="event.stopPropagation();discDev(\''+esc(path)+'\')">&#215;</button>';
tab.onclick=function(){swTerm2(path)};tabs.appendChild(tab);
var term=document.createElement('div');term.className='tp';term.id='tp-'+eId(path);
term.innerHTML='<div class="th"><span>'+esc(name)+'</span><span class="ri" onclick="rmTerm(\''+esc(path)+'\')">&#10005;</span></div>'
+'<div class="tb" id="tb-'+eId(path)+'"></div>'
+'<div class="ti"><span class="pm">$</span><input type="text" id="inp-'+eId(path)+'" placeholder="输入命令按Enter" onkeydown="sendCmd(event,\''+esc(path)+'\')"></div>';
document.getElementById('terminals').appendChild(term);swTerm2(path);renderTerm(path)}
function updTH(path,name){var p=document.getElementById('tp-'+eId(path));if(!p)return;var h=p.querySelector('.th span');if(h)h.textContent=name;var t=document.getElementById('t-'+eId(path));if(t)t.querySelector('span').textContent=name}
...
var cols=Math.ceil(Math.sqrt(mainNodes.length));
mainNodes.forEach(function(n,i){pos[n.id]={x:60+(i%cols)*((w-120)/Math.max(cols-1,1)),y:40+Math.floor(i/cols)*((h-80)/Math.max(Math.ceil(mainNodes.length/cols)-1,1))}});
