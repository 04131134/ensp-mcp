...
def test_device_verify_returns_checks_list(monkeypatch):
    ...
    monkeypatch.setattr(flask_app.services.commands, "send", fake_send)
    monkeypatch.setattr(flask_app.services.device_manager, "has", lambda path: path == "127.0.0.1:2001")
...
def test_device_verify_connectivity_with_target(monkeypatch):
    ...
    monkeypatch.setattr(flask_app.services.commands, "send", fake_send)
    monkeypatch.setattr(flask_app.services.device_manager, "has", lambda path: path == "127.0.0.1:2002")
    client = flask_app.app.test_client()
    resp = client.post(
        "/api/devices/verify",
